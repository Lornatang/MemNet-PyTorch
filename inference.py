...
def main(args):
    ...
    # Load the MemNet model weights
    checkpoint = torch.load(args.weights_path, map_location=lambda storage, loc: storage)
    model.load_state_dict(checkpoint["state_dict"])
    print(f"Load MemNet model weights `{args.weights_path}` successfully.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Using the MemNet model generator super-resolution images.")
